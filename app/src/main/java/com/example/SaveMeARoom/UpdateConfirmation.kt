...
class UpdateConfirmation : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnConfirm.setOnClickListener {
            ...
            val checkResQuery = "/search?query=SELECT%20*%20FROM%20reservations%20WHERE%20Reserver_Email=%27" + email + "%27%20AND%20Start_Date_Time=%27"  + date + "%20" + start + "%27"
            val checkurl = URL(ip.plus(checkResQuery))
            val checkText = checkurl.readText()

            if(checkText.length > 2){
                Toast.makeText(this, "You already have a reservation at this time.", Toast.LENGTH_SHORT).show()
            }else{
